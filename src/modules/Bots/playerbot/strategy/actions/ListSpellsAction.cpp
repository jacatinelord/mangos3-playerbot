...
bool ListSpellsAction::Execute(Event event)
{
    ...
    for (PlayerSpellMap::iterator itr = bot->GetSpellMap().begin(); itr != bot->GetSpellMap().end(); ++itr)
    {
        ...
        {
            const SpellReagentsEntry* const spellreagent = pSpellInfo->GetSpellReagents();
            if (spellreagent->Reagent[x] <= 0)
            {
                continue;
            }

            uint32 itemid = spellreagent->Reagent[x];
            uint32 reagentsRequired = spellreagent->ReagentCount[x];
            if (itemid)
            {
        ...
        if (skillLine)
        {
            for (int i = 0; i < 3; ++i)
            {
                const SpellEffectEntry* spelleffect = pSpellInfo->GetSpellEffect(SpellEffectIndex(i));
                if (spelleffect && spelleffect->Effect == SPELL_EFFECT_CREATE_ITEM)
                {
                    ItemPrototype const* proto = sObjectMgr.GetItemPrototype(spelleffect->EffectItemType);
                    if (proto)
                    {
                        if (craftCount)
                        {
