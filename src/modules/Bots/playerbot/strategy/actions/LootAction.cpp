...
bool OpenLootAction::CanOpenLock(LootObject& lootObject, const SpellEntry* pSpellInfo, GameObject* go)
{
    ...
    {
        const SpellEffectEntry* spelleffect = pSpellInfo->GetSpellEffect(SpellEffectIndex(effIndex));
        if (spelleffect->Effect != SPELL_EFFECT_OPEN_LOCK && spelleffect->Effect != SPELL_EFFECT_SKINNING)
        {
            return false;
        }
