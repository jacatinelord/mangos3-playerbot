...
int AhBot::AddAuction(int auction, Category* category, ItemPrototype const* proto)
{
    ...
    string name;
    ObjectGuid playerGuid = ObjectGuid(HIGHGUID_PLAYER, owner);
    // Player* player_m = sObjectMgr.GetPlayer(playerGuid);
    if (!sObjectMgr.GetPlayerNameByGUID(playerGuid, name))
    {
        return 0;
    }
...
void AhBot::CheckSendMail(uint32 bidder, uint32 price, AuctionEntry *entry)
{
    ...
    string name;
    ObjectGuid playerGuid = ObjectGuid(HIGHGUID_PLAYER, bidder);
    // Player* player_m = sObjectMgr.GetPlayer(playerGuid);
    if (!sObjectMgr.GetPlayerNameByGUID(playerGuid, name))
    {
        return;
