...
{
    const SpellEntry* const entry = sSpellStore.LookupEntry(spellId);
    ...
    {
        const SpellReagentsEntry* const spellreagent = entry->GetSpellReagents();
        if (spellreagent->Reagent[x] <= 0)
        {
            continue;
        }

        if (proto->ItemId == spellreagent->Reagent[x])
        {
...
bool TradeSkill::IsCraftedBy(ItemPrototype const* proto, uint32 spellId)
{
    ...
    {
        const SpellEffectEntry* const entryeffect = entry->GetSpellEffect(SpellEffectIndex(effect));
        uint32 craftId = entryeffect->EffectTriggerSpell;
        ...
        {
            const SpellReagentsEntry* const craftreagent = craft->GetSpellReagents();
            uint32 itemId = craftreagent->Reagent[i];
