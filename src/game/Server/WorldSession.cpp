...
WorldSession::WorldSession(uint32 id, WorldSocket *sock, AccountTypes sec, uint8 expansion, time_t mute_time, LocaleConstant locale) : m_muteTime(mute_time), _player(NULL), m_Socket(sock), _security(sec), _accountId(id), m_expansion(expansion), _logoutTime(0),
...
void WorldSession::SendPacket(WorldPacket const *packet)
{
    ...
    if (GetPlayer())
    {
        if (packet)
        {
            sLog.outString(">> Sending Packet: Opcode = %u (0x%X), Size = %zu", packet->GetOpcode(), packet->GetOpcode(), packet->size());
        }

        if (GetPlayer()->GetPlayerbotAI())
        {
            sLog.outString(">> Logged As GetPlayerbotAI");
            GetPlayer()->GetPlayerbotAI()->HandleBotOutgoingPacket(*packet);
        }
        else if (GetPlayer()->GetPlayerbotMgr())
        {
            sLog.outString(">> Logged As GetPlayerbotMgr");
